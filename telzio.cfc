...
*/
component displayname="telziocfc"  {

    variables._telziocfc_version = '0.0.0';

    public any function init(
        string api_key = '',
        string api_secret = '',
        string baseUrl = "https://api.telzio.com",
        ...
        var secrets = {
            'api_key': 'TELZIO_API_KEY',
            'api_secret': 'TELZIO_API_SECRET'
        };
    ...
    /**
    * @docs https://developer.telzio.com/reference#balance
    * @hint Retrieve your current account balance in USD
    */
    public struct function getAccountBalance() {
      return apiCall( 'GET', '/account/balance' );
    }

    /**
    * @docs https://developer.telzio.com/reference#dial-outbound-number
    * @hint Dials an external phone number, and when answered connects it to an extension, username or another external phone number.
    * @aleg The E164 formatted number in your Telzio account to dial from. This call will be charged towards this number's plan For example: +18889989080
    * @bleg The extension, username or E164 formatted phone number to dial when Aleg has been answered.
    */
    public struct function dialExternalPhone( required string from, required string aleg, required string bleg ) {
      var payload = arguments;
      return apiCall( 'POST', '/dial/outbound', {}, payload );
    }
    ...
    */
    ...
    private struct function getBaseHttpHeaders() {
        return {
            ...
            'User-Agent' : 'telziocfc/#variables._telziocfc_version# (ColdFusion)'
        };
    }
    ...
    private any function makeHttpRequest(
        ...
        cfhttp( url = fullPath, method = httpMethod, username = variables.api_key, password = variables.api_secret, result = 'result', timeout = variables.httpTimeout ) {
